...
if [ -z "${validation_cpg}" ]; then
    fail "module16_positive_control_cpg is empty. Please set it in your config before running 16e."
fi

check_file "${reference_file}"

echo "Running 16e Module 16 HASE meta-classic positive-control validation"
...
run_sex_hase_validation() {
    ...
    echo "Running ${sex_label} Module 16 HASE positive-control validation"
    if [ ! -d "${meta_inputs}" ]; then
        echo "Skipping ${sex_label}: meta input directory is missing: ${meta_inputs}"
        return 0
    fi
    ...
    python "${light_hase}/hase.py" \
        ...
        -cluster n
    if [ "$?" -ne "0" ]; then
        fail "light_hase meta-classic failed for ${sex_label} Module 16 positive control"
    fi
...
PY
    if [ "$?" -ne "0" ]; then
        fail "Combining HASE validation feather outputs failed for ${sex_label}"
    fi
...
if [ "${sex_hase_validation_count}" -eq "0" ]; then
    fail "No sex-specific Module 16 positive-control HASE validations were completed."
fi

echo "16e Module 16 HASE positive-control validation successfully completed"
