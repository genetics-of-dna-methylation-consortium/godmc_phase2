    ...
    if command -v mamba &> /dev/null; then
        if mamba env list | grep -qE 'hail_env(\s|$)'; then
            ...
            if conda env list | grep -qE 'hail_env(\s|$)'; then
                ...
                echo "ERROR: hail_env environment does not exist in mamba or conda. Please ensure hail_env exists."
                exit 1
            fi
        else
            echo "ERROR: hail_env environment does not exist in mamba, and conda not found."
            exit 1
        fi
    elif command -v conda &> /dev/null; then
        if conda env list | grep -qE 'hail_env(\s|$)'; then
            ...
            echo "ERROR: hail_env environment does not exist in conda. Please ensure hail_env exists."
            exit 1
        fi
    else
