...
## Remove SNPs with duplicated positions
snps$map$ChrPos <- paste(snps$map$chromosome, snps$map$position)
rep.pos <- snps$map$ChrPos[duplicated(snps$map$ChrPos)]

bad.SNPs <- snps$map$ChrPos %in% rep.pos

snps$genotypes <- snps$genotypes[, !bad.SNPs]
snps$map <- snps$map[!bad.SNPs, ]
...
invsList[[1]] <- invsList[[1]][ids]
invsDF <- Reduce(function(x, y) rbind(x, y[ids]), invsList)
colnames(invsDF) <- ids
bad_freqs <- rowMeans(invsDF == "0",na.rm = TRUE) == 1 | rowMeans(invsDF == "2", na.rm = TRUE) == 1
...
## Remove inversions where all genotypes are the same
bad_freq_invs <- invsDF[bad_freqs, 1]
invsDF <- invsDF[!bad_freqs, ]
...
df <- data.frame(inversions = c(errorInvs, snpInvs, genoInvs, bad_freq_invs), 
                 Reason = c(errors, rep(c("Not enough SNPs", "Bad genotyping", "Invariant Inversions"), 
                              c(length(snpInvs), length(genoInvs), length(bad_freq_invs)))))
write.table(df, file = paste0(res_folder, "/badInversions.txt"), sep = "\t",
            row.names = FALSE, col.names = TRUE, quote = FALSE)

invFreqs <- round(rowMeans(invsDF, na.rm = TRUE)/2*100, 2)

n_SNPs <- sapply(invstat2[names(invFreqs)], function(x) max(numSNPs(x)))

df <- data.frame(inversion = names(invFreqs), freq = invFreqs, n_SNPs = n_SNPs, N = rowSums(!is.na(invsDF)))
